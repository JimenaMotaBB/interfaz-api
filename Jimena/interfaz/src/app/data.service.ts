...
export class DataService {
  ...
  private goals = new BehaviorSubject<any>(['Series']);
  ...
  apiURL = 'http://34.125.7.41:8122/series-api';
  ...
  getGoals(): Observable<GoalsApi> {
    return this.http.get<GoalsApi>(this.apiURL + '/series', this.httpOptions)
    .pipe(
      retry(1),
  ...
  newGoal(payload): Observable<GoalsApi> {
    return this.http.post<GoalsApi>(this.apiURL + '/series', JSON.stringify(payload), this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
